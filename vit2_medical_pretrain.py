...
SAVE_DIR = "/home/fuxiangyu/jlx/MVI/MVI0325_test/pth/pth_vit2_medical"
...
class MultiModalViT(nn.Module):
    def __init__(self, num_clinical_features, num_classes=2):
        ...
        # 1. 图像分支: ViT with 医学影像预训练权重（全冻结策略）
        # 尝试加载医学影像预训练权重，如果失败则回退到 ImageNet
        print("▶️ 尝试加载医学影像预训练权重...")

        medical_pretrain_loaded = False

        # 方案1: 尝试使用 timm 库加载 RadImageNet 预训练的 ViT
        try:
            import timm
            print("  尝试方案1: timm + RadImageNet...")
            # 注意：这需要手动下载 RadImageNet 预训练权重
            # 下载地址: https://github.com/BMEII-AI/RadImageNet
            self.vit = timm.create_model('vit_base_patch16_224', pretrained=False, num_classes=0)

            # 尝试加载本地的 RadImageNet 权重（如果存在）
            radimagenet_path = "/home/fuxiangyu/jlx/MVI/pretrained_weights/radimagenet_vit_base_patch16_224.pth"
            if os.path.exists(radimagenet_path):
                print(f"  ✅ 找到 RadImageNet 权重: {radimagenet_path}")
                state_dict = torch.load(radimagenet_path, map_location='cpu')
                self.vit.load_state_dict(state_dict, strict=False)
                self.vit_hidden_dim = 768
                medical_pretrain_loaded = True
                print("  ✅ 成功加载 RadImageNet 预训练权重")
            else:
                print(f"  ⚠️  未找到 RadImageNet 权重文件: {radimagenet_path}")
        except Exception as e:
            print(f"  ⚠️  方案1失败: {e}")

        # 方案2: 尝试使用 Hugging Face 的医学影像预训练模型
        if not medical_pretrain_loaded:
            try:
                from transformers import ViTModel, ViTConfig
                print("  尝试方案2: Hugging Face + 医学预训练...")

                # 尝试加载 microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224
                # 或其他医学影像预训练的 ViT
                model_name = "microsoft/swin-tiny-patch4-window7-224"  # 示例，需要替换为实际的医学预训练模型

                # 注意：这里需要实际的医学预训练模型名称
                # 可选项：
                # - microsoft/BiomedCLIP (需要特殊处理)
                # - flaviagiammarino/pubmed-clip-vit-base-patch32 (PubMed 预训练)

                print("  ⚠️  Hugging Face 医学预训练模型需要手动配置")
                medical_pretrain_loaded = False
            except Exception as e:
                print(f"  ⚠️  方案2失败: {e}")

        # 回退方案: 使用 ImageNet 预训练权重
        if not medical_pretrain_loaded:
            print("  ⚠️  所有医学预训练方案失败，回退到 ImageNet 预训练")
            print("  提示：要使用医学预训练权重，请：")
            print("    1. 下载 RadImageNet 权重到: /home/fuxiangyu/jlx/MVI/pretrained_weights/")
            print("    2. 或安装并配置 Hugging Face transformers 库")
            self.vit = models.vit_b_16(weights=models.ViT_B_16_Weights.IMAGENET1K_V1)
            self.vit_hidden_dim = self.vit.hidden_dim  # 768
            self.vit.heads = nn.Identity()
            print("  ✅ 使用 ImageNet 预训练权重（回退方案）")
