...
# ==========================================
# 3. 多模态融合网络模型 (ResNet50 版 - RadImageNet)
# ==========================================
class MultiModalResNet50(nn.Module):
    def __init__(self, num_clinical_features, num_classes=2):
        ...
        # 1. 图像分支: ResNet50 with RadImageNet 预训练权重
        print("▶️ 尝试加载 RadImageNet ResNet50 预训练权重...")

        radimagenet_loaded = False
        radimagenet_path = "/home/fuxiangyu/jlx/MVI/pretrained_weights/RadImageNet-ResNet50_notop.h5"

        # 尝试加载 RadImageNet 权重
        if os.path.exists(radimagenet_path):
            print(f"  ✅ 找到 RadImageNet 权重: {radimagenet_path}")
            # 注意：RadImageNet 提供的是 Keras/TensorFlow 格式 (.h5)
            # 需要转换为 PyTorch 格式，或者使用 PyTorch 版本的权重
            print("  ⚠️  RadImageNet 权重是 Keras 格式，需要转换为 PyTorch")
            print("  提示：请下载 PyTorch 版本的 RadImageNet ResNet50 权重")
            radimagenet_loaded = False
        else:
            print(f"  ⚠️  未找到 RadImageNet 权重: {radimagenet_path}")

        # 加载 ResNet50 模型
        if radimagenet_loaded:
            # 如果有 RadImageNet 权重，加载它
            self.resnet = models.resnet50(weights=None)
            # 这里需要加载 RadImageNet 权重的代码
            # state_dict = torch.load(radimagenet_path)
            # self.resnet.load_state_dict(state_dict, strict=False)
            print("  ✅ 使用 RadImageNet 预训练权重")
        else:
            # 回退到 ImageNet 预训练
            print("  ⚠️  回退到 ImageNet 预训练权重")
            self.resnet = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V1)
            print("  ✅ 使用 ImageNet ResNet50 预训练权重（回退方案）")

        self.resnet_hidden_dim = self.resnet.fc.in_features  # 2048 (ResNet50)
        ...
        print(f"✅ ResNet50 全部冻结，特征维度: {self.resnet_hidden_dim}")
        ...
        # 3. 融合分类头: 2048 + 64 = 2112 维
        fused_dim = self.resnet_hidden_dim + 64
    ...
    def forward(self, image, clinical_data):
        img_features = self.resnet(image)            # [B, 2048]
        clin_features = self.clinical_mlp(clinical_data) # [B, 64]
        fused_features = torch.cat((img_features, clin_features), dim=1) # [B, 2112]
