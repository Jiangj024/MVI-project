# -*- coding: utf-8 -*-
"""
ResNet50 + RadImageNet for MVI Prediction
使用 RadImageNet 医学影像预训练权重的 ResNet50 模型
"""
...
RADIMAGENET_PATH = "/home/fuxiangyu/jlx/MVI/pretrained_weights/ResNet50.pt"
...
SAVE_DIR = "/home/fuxiangyu/jlx/MVI/MVI0325_test/pth/pth_resnet50_radimagenet"
...
class MultiModalResNet50(nn.Module):
    def __init__(self, num_clinical_features, num_classes=2):
        ...
        print("▶️ 加载 RadImageNet ResNet50 预训练权重...")
        ...
        missing_keys, unexpected_keys = self.resnet.load_state_dict(new_state_dict, strict=False)
        print(f"  ✅ 成功加载 RadImageNet 权重 (Missing: {len(missing_keys)}, Unexpected: {len(unexpected_keys)})")
        
        self.resnet_hidden_dim = self.resnet.fc.in_features  # 2048
        ...
        # 融合分类头
        fused_dim = self.resnet_hidden_dim + 64  # 2048 + 64 = 2112
...
def train_multimodal_resnet50():
    ...
    for fold, (train_p_idx, test_p_idx) in enumerate(kf.split(unique_patients)):
        ...
        model = MultiModalResNet50(num_clinical_features=num_features, num_classes=2).to(DEVICE)
        ...
        for epoch in range(EPOCHS):
            ...
            if val_auc_patient > best_auc:
                ...
                torch.save(model.state_dict(), os.path.join(SAVE_DIR, f"best_resnet50_radimagenet_fold{fold+1}.pth"))
    ...
    print("\n" + "="*50)
    print("🏆 ResNet50 + RadImageNet 5-Fold 交叉验证最终结果")
