# -*- coding: utf-8 -*-
"""
ViT-Small for MVI Prediction
使用 ViT-Small (标准 ViT 架构的轻量版本)
基于 ViT-Small 图像特征与 21 维临床表格数据的晚期融合
"""
...
SAVE_DIR = "/home/fuxiangyu/jlx/MVI/MVI0325_test/pth/pth_vit_small"
...
# 模型定义
class MultiModalViTSmall(nn.Module):
    def __init__(self, num_clinical_features, num_classes=2):
        super(MultiModalViTSmall, self).__init__()
        
        print("▶️ 加载 ViT-Small 预训练权重（ImageNet）...")
        
        # 使用 timm 加载 ViT-Small
        self.vit = timm.create_model('vit_small_patch16_224', pretrained=True, num_classes=0)
        self.vit_hidden_dim = 384  # ViT-Small 输出 384 维特征
        
        print(f"  ✅ 成功加载 ViT-Small 预训练权重")
        print(f"  📊 ViT-Small 参数: ~22M, 特征维度: {self.vit_hidden_dim}")
        
        # 全冻结
        for param in self.vit.parameters():
            param.requires_grad = False
        
        print(f"✅ ViT-Small 全部冻结，可训练参数: ~0.05M (仅分类头)")
        ...
        # 融合分类头
        fused_dim = self.vit_hidden_dim + 64  # 384 + 64 = 448
...
# 训练主流程
def train_multimodal_vit_small():
    ...
    for fold, (train_p_idx, test_p_idx) in enumerate(kf.split(unique_patients)):
        ...
        model = MultiModalViTSmall(num_clinical_features=num_features, num_classes=2).to(DEVICE)
        ...
        for epoch in range(EPOCHS):
            ...
            if val_auc_patient > best_auc:
                ...
                torch.save(model.state_dict(), os.path.join(SAVE_DIR, f"best_vit_small_fold{fold+1}.pth"))
    ...
    print("\n" + "="*50)
    print("🏆 ViT-Small 5-Fold 交叉验证最终结果")
